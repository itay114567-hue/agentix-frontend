...
function InteractiveDemo() {
  const [activeTab, setActiveTab] = useState<'chat' | 'voice' | 'email'>('chat');
  ...
  const voiceTranscript = [
    { role: "user", text: "Hi, I need to cancel my subscription." },
    { role: "agent", text: "I understand. Before we proceed, may I ask the reason for cancellation?" },
    { role: "user", text: "It's too expensive for me right now." },
    { role: "agent", text: "I completely understand. I can offer you a 40% discount for the next 3 months. Would that help?" },
    { role: "user", text: "Actually, yes! That sounds great." },
    { role: "agent", text: "Perfect! I've applied the discount. You're all set — enjoy the savings!" },
  ];

  const emailThread = [
    {
      from: "customer@company.com",
      subject: "Invoice #8821 — incorrect amount",
      preview: "Hello, I received invoice #8821 but the amount seems wrong. We agreed on $2,400 but I was charged $3,100...",
      time: "9:14 AM",
      status: "incoming",
    },
    {
      from: "Aria — AI Support",
      subject: "Re: Invoice #8821 — incorrect amount",
      preview: "Hi! I've reviewed invoice #8821 and you're right — there was a billing error. I've issued a corrected invoice for $2,400 and a credit note for the difference. Please check your inbox.",
      time: "9:14 AM",
      status: "outgoing",
      badge: "Auto-resolved · 0s",
    },
  ];

  // Reset and replay on tab change
  useEffect(() => {
    setChatStep(0);
    setVoiceStep(0);
    setEmailStep(0);
    ...
    const t1 = setTimeout(() => {
      if (activeTab === 'chat') setChatStep(1);
      if (activeTab === 'voice') { setVoiceActive(true); setVoiceStep(1); }
      if (activeTab === 'email') setEmailStep(1);
    }, 400);
  ...
  useEffect(() => {
    ...
    if (chatStep > 0 && chatStep < chatConversation.length) {
      const t = setTimeout(() => setChatStep(s => s + 1), 1400);
  ...
  useEffect(() => {
    ...
    if (voiceStep > 0 && voiceStep < voiceTranscript.length) {
      const t = setTimeout(() => setVoiceStep(s => s + 1), 1600);
      return () => clearTimeout(t);
    }
  }, [voiceStep, activeTab]);
  ...
  useEffect(() => {
    ...
    if (emailStep === 1) {
      const t = setTimeout(() => setEmailStep(2), 1800);
      return () => clearTimeout(t);
    }
  }, [emailStep, activeTab]);
  ...
  const tabConfig = {
    ...
    voice: { label: "Voice", icon: PhoneCall, color: "#8b5cf6", metric1: "99.7%", label1: "Accuracy", metric2: "0.3s", label2: "Response time" },
    email: { label: "Email", icon: Mail, color: "#10b981", metric1: "95%", label1: "Auto-resolved", metric2: "< 1s", label2: "Response time" },
  };
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start max-w-6xl mx-auto">
          ...
          <div className="reveal">
            <div className="flex gap-3 mb-8">
              {(['chat', 'voice', 'email'] as const).map((t) => {
                const Ic = tabConfig[t].icon;
                return (
                  <button
                    key={t}
                    onClick={() => setActiveTab(t)}
                    className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold transition-all text-sm ${
                      activeTab === t ? 'text-white shadow-lg' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                    }`}
                    style={{
                      fontFamily: "Space Grotesk, sans-serif",
                      background: activeTab === t ? tabConfig[t].color : undefined,
                    }}
                  >
                    <Ic size={15} />
                    {tabConfig[t].label}
            ...
            <div className="bg-white rounded-2xl p-8 border border-gray-200 shadow-sm">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: `${tab.color}18` }}>
                  <TabIcon size={20} style={{ color: tab.color }} />
                </div>
                <div>
                  <h3 className="text-xl font-bold text-[#050508]" style={{ fontFamily: "Space Grotesk, sans-serif" }}>
                    {activeTab === 'chat' && "Live Chat Agent"}
                    {activeTab === 'voice' && "Voice Support Agent"}
                    {activeTab === 'email' && "Email Automation"}
                  </h3>
                  <p className="text-xs text-gray-500" style={{ fontFamily: "Inter, sans-serif" }}>
                    {activeTab === 'chat' && "Real-time conversational AI across web & mobile"}
                    {activeTab === 'voice' && "Natural language voice with 99.7% accuracy"}
                    {activeTab === 'email' && "Intelligent triage & automated responses"}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-2.5 mb-6">
                {activeTab === 'chat' && (
                  <>
                    {["Instant replies with context awareness", "Sentiment analysis & escalation detection", "Seamless handoff to human agents"].map((p, i) => (
                      <div key={i} className="flex items-center gap-2.5"><Check size={16} style={{ color: tab.color }} /><span className="text-gray-700 text-sm" style={{ fontFamily: "Inter, sans-serif" }}>{p}</span></div>
                    ))}
                  </>
                )}
                {activeTab === 'voice' && (
                  <>
                    {["40+ languages with emotion detection", "IVR replacement — zero hold times", "Full call transcription & summary"].map((p, i) => (
                      <div key={i} className="flex items-center gap-2.5"><Check size={16} style={{ color: tab.color }} /><span className="text-gray-700 text-sm" style={{ fontFamily: "Inter, sans-serif" }}>{p}</span></div>
                    ))}
                  </>
                )}
                {activeTab === 'email' && (
                  <>
                    {["95% auto-resolution — no human needed", "Smart triage & priority routing", "Template learning from your best responses"].map((p, i) => (
                      <div key={i} className="flex items-center gap-2.5"><Check size={16} style={{ color: tab.color }} /><span className="text-gray-700 text-sm" style={{ fontFamily: "Inter, sans-serif" }}>{p}</span></div>
                    ))}
                  </>
                )}
              ...
              <div className="grid grid-cols-2 gap-3 pt-5 border-t border-gray-100">
                <div className="bg-gray-50 rounded-xl px-4 py-3">
                  <p className="text-2xl font-bold" style={{ color: tab.color, fontFamily: "Space Grotesk, sans-serif" }}>{tab.metric1}</p>
                  <p className="text-xs text-gray-500 mt-0.5" style={{ fontFamily: "Inter, sans-serif" }}>{tab.label1}</p>
                </div>
                <div className="bg-gray-50 rounded-xl px-4 py-3">
                  <p className="text-2xl font-bold" style={{ color: tab.color, fontFamily: "Space Grotesk, sans-serif" }}>{tab.metric2}</p>
                  <p className="text-xs text-gray-500 mt-0.5" style={{ fontFamily: "Inter, sans-serif" }}>{tab.label2}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="reveal">
            {/* CHAT widget */}
            {activeTab === 'chat' && (
              <div className="bg-[#0D0D14] border border-white/12 rounded-2xl overflow-hidden shadow-2xl">
                <div className="flex items-center gap-3 px-5 py-4 border-b border-white/8">
                  <div className="w-9 h-9 bg-[#2563EB] rounded-full flex items-center justify-center">
                    <Bot size={18} className="text-white" />
                  </div>
                  <div>
                    <p className="text-white font-semibold text-sm" style={{ fontFamily: "Space Grotesk, sans-serif" }}>Aria — AI Support Agent</p>
                ...
                <div className="p-5 space-y-3.5 min-h-[280px]">
                  {chatConversation.slice(0, chatStep).map((msg, i) => (
                    <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"} animate-fade-in-up`}>
                      ...
                      <div className={`max-w-[80%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${
                        msg.role === "user" ? "bg-white/12 text-white rounded-tr-sm" : "bg-[#2563EB]/20 text-white/90 rounded-tl-sm border border-[#2563EB]/20"
                      }`} style={{ fontFamily: "Inter, sans-serif" }}>
                        {msg.text}
                      </div>
                    </div>
                  ))}
                  {chatStep > 0 && chatStep < chatConversation.length && (
                    <div className="flex justify-start">
                      <div className="w-7 h-7 bg-[#2563EB] rounded-full flex items-center justify-center mr-2 mt-0.5"><Bot size={13} className="text-white" /></div>
                      <div className="bg-[#2563EB]/20 border border-[#2563EB]/20 px-4 py-3 rounded-2xl rounded-tl-sm flex gap-1">
                        {[0,1,2].map(i => <div key={i} className="w-1.5 h-1.5 bg-[#60A5FA] rounded-full animate-bounce" style={{ animationDelay: `${i*0.15}s` }} />)}
                      </div>
                    </div>
                  )}
                </div>
                <div className="px-5 pb-5">
                  <div className="flex items-center gap-3 bg-white/6 border border-white/10 rounded-xl px-4 py-3">
                    <span className="text-white/30 text-sm flex-1" style={{ fontFamily: "Inter, sans-serif" }}>Type a message...</span>
                    <div className="flex gap-2"><Headphones size={16} className="text-white/30" /><MessageSquare size={16} className="text-[#2563EB]" /></div>
                  </div>
                </div>
                <div className="flex gap-3 px-5 pb-5">
                  <div className="flex-1 bg-[#0D0D14] border border-white/10 rounded-xl px-4 py-3 flex items-center gap-3">
                    <TrendingUp size={15} className="text-green-400" />
                    <div><p className="text-white font-bold text-base leading-none" style={{ fontFamily: "Space Grotesk, sans-serif" }}>94.7%</p><p className="text-white/40 text-xs mt-0.5">Resolution rate</p></div>
                  </div>
                  <div className="flex-1 bg-[#0D0D14] border border-white/10 rounded-xl px-4 py-3 flex items-center gap-3">
                    <Clock size={15} className="text-blue-400" />
                    <div><p className="text-white font-bold text-base leading-none" style={{ fontFamily: "Space Grotesk, sans-serif" }}>0.3s</p><p className="text-white/40 text-xs mt-0.5">Avg response</p></div>
                  </div>
                </div>
              </div>
            )}
            ...
            {activeTab === 'voice' && (
              <div className="bg-[#0D0D14] border border-white/12 rounded-2xl overflow-hidden shadow-2xl">
                <div className="flex items-center gap-3 px-5 py-4 border-b border-white/8">
                  ...
                  <div>
                    <p className="text-white font-semibold text-sm" style={{ fontFamily: "Space Grotesk, sans-serif" }}>Aria — Voice Agent</p>
                    <div className="flex items-center gap-1.5">
                      <div className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse" />
                      <span className="text-white/45 text-xs">Live call · 0:32</span>
                    </div>
                  </div>
                    ...
                    {voiceActive && (
                      <div className="flex items-end gap-0.5 h-5">
                        {[3,5,4,6,3,5,4,3,6,5].map((h, i) => (
                          <div key={i} className="w-1 rounded-full bg-[#8b5cf6] animate-bounce" style={{ height: `${h * 3}px`, animationDelay: `${i * 0.08}s` }} />
                        ))}
                      </div>
                    )}
                  </div>
                ...
                <div className="px-5 pt-5 pb-3">
                  <div className="bg-[#8b5cf6]/10 border border-[#8b5cf6]/20 rounded-xl p-4 flex items-center justify-center gap-1 h-16">
                ...
                {/* Transcript */}
                <div className="px-5 pb-4 space-y-3 min-h-[200px]">
                  <p className="text-white/40 text-xs uppercase tracking-wider mb-3" style={{ fontFamily: "Inter, sans-serif" }}>Live Transcript</p>
                  {voiceTranscript.slice(0, voiceStep).map((msg, i) => (
                    <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"} animate-fade-in-up`}>
                      {msg.role === "agent" && (
                        <div className="w-6 h-6 bg-[#8b5cf6] rounded-full flex items-center justify-center mr-2 mt-0.5 flex-shrink-0">
                          <Bot size={11} className="text-white" />
                        </div>
                      )}
                      <div className={`max-w-[80%] px-3 py-2 rounded-xl text-xs leading-relaxed ${
                        msg.role === "user" ? "bg-white/10 text-white/80 rounded-tr-sm" : "bg-[#8b5cf6]/20 text-white/90 rounded-tl-sm border border-[#8b5cf6]/20"
                      }`} style={{ fontFamily: "Inter, sans-serif" }}>
                        {msg.text}
                      </div>
                    </div>
                  ))}
                  {voiceStep > 0 && voiceStep < voiceTranscript.length && (
                    <div className="flex justify-start">
                      <div className="w-6 h-6 bg-[#8b5cf6] rounded-full flex items-center justify-center mr-2"><Bot size={11} className="text-white" /></div>
                      <div className="bg-[#8b5cf6]/20 border border-[#8b5cf6]/20 px-3 py-2 rounded-xl flex gap-1">
                        {[0,1,2].map(i => <div key={i} className="w-1.5 h-1.5 bg-[#a78bfa] rounded-full animate-bounce" style={{ animationDelay: `${i*0.15}s` }} />)}
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="flex gap-3 px-5 pb-5">
                  <div className="flex-1 bg-[#0D0D14] border border-white/10 rounded-xl px-4 py-3 flex items-center gap-3">
                    ...
                    <div><p className="text-white font-bold text-base leading-none" style={{ fontFamily: "Space Grotesk, sans-serif" }}>40+</p><p className="text-white/40 text-xs mt-0.5">Languages</p></div>
                  </div>
                  <div className="flex-1 bg-[#0D0D14] border border-white/10 rounded-xl px-4 py-3 flex items-center gap-3">
                    <Zap size={15} className="text-yellow-400" />
                    <div><p className="text-white font-bold text-base leading-none" style={{ fontFamily: "Space Grotesk, sans-serif" }}>99.7%</p><p className="text-white/40 text-xs mt-0.5">Accuracy</p></div>
                  </div>
                </div>
              </div>
            )}
            ...
            {activeTab === 'email' && (
              <div className="bg-[#0D0D14] border border-white/12 rounded-2xl overflow-hidden shadow-2xl">
                <div className="flex items-center gap-3 px-5 py-4 border-b border-white/8">
                  ...
                  <div>
                    <p className="text-white font-semibold text-sm" style={{ fontFamily: "Space Grotesk, sans-serif" }}>Aria — Email Agent</p>
                    <div className="flex items-center gap-1.5">
                      <div className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse" />
                      <span className="text-white/45 text-xs">Inbox · 3 new</span>
                    </div>
                  </div>
                  <div className="ml-auto flex gap-1.5">
                  ...
                  {emailStep >= 1 && (
                    <div className="animate-fade-in-up bg-white/5 border border-white/10 rounded-xl p-4">
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          <p className="text-xs text-white/40 mb-0.5" style={{ fontFamily: "Inter, sans-serif" }}>FROM: {emailThread[0].from}</p>
                          <p className="text-white font-semibold text-sm" style={{ fontFamily: "Space Grotesk, sans-serif" }}>{emailThread[0].subject}</p>
                        </div>
                        <span className="text-white/30 text-xs">{emailThread[0].time}</span>
                      </div>
                      <p className="text-white/55 text-xs leading-relaxed" style={{ fontFamily: "Inter, sans-serif" }}>{emailThread[0].preview}</p>
                      <div className="flex items-center gap-2 mt-3">
                        <span className="text-xs px-2 py-0.5 rounded-full bg-orange-500/20 text-orange-400 border border-orange-500/20">Billing Issue</span>
                        <span className="text-xs px-2 py-0.5 rounded-full bg-red-500/20 text-red-400 border border-red-500/20">High Priority</span>
                      </div>
                    </div>
                  )}

                  {emailStep < 2 && emailStep >= 1 && (
                    <div className="flex items-center gap-3 bg-[#10b981]/10 border border-[#10b981]/20 rounded-xl px-4 py-3 animate-fade-in-up">
                      <div className="flex gap-1">
                        {[0,1,2].map(i => <div key={i} className="w-1.5 h-1.5 bg-[#10b981] rounded-full animate-bounce" style={{ animationDelay: `${i*0.15}s` }} />)}
                      </div>
                      <p className="text-[#10b981] text-xs" style={{ fontFamily: "Inter, sans-serif" }}>Aria is drafting a response...</p>
                    </div>
                  )}

                  {emailStep >= 2 && (
                    <div className="animate-fade-in-up bg-[#10b981]/10 border border-[#10b981]/25 rounded-xl p-4">
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          <p className="text-xs text-[#10b981]/70 mb-0.5" style={{ fontFamily: "Inter, sans-serif" }}>FROM: {emailThread[1].from}</p>
                          <p className="text-white font-semibold text-sm" style={{ fontFamily: "Space Grotesk, sans-serif" }}>{emailThread[1].subject}</p>
                        </div>
                        <span className="text-white/30 text-xs">{emailThread[1].time}</span>
                      </div>
                      <p className="text-white/70 text-xs leading-relaxed" style={{ fontFamily: "Inter, sans-serif" }}>{emailThread[1].preview}</p>
                      <div className="flex items-center gap-2 mt-3">
                        <span className="text-xs px-2 py-0.5 rounded-full bg-[#10b981]/20 text-[#10b981] border border-[#10b981]/30">✓ {emailThread[1].badge}</span>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="flex gap-3 px-5 pb-5">
                  <div className="flex-1 bg-[#0D0D14] border border-white/10 rounded-xl px-4 py-3 flex items-center gap-3">
                    ...
                    <div><p className="text-white font-bold text-base leading-none" style={{ fontFamily: "Space Grotesk, sans-serif" }}>95%</p><p className="text-white/40 text-xs mt-0.5">Auto-resolved</p></div>
                  </div>
                  <div className="flex-1 bg-[#0D0D14] border border-white/10 rounded-xl px-4 py-3 flex items-center gap-3">
                    <Zap size={15} className="text-yellow-400" />
                    <div><p className="text-white font-bold text-base leading-none" style={{ fontFamily: "Space Grotesk, sans-serif" }}>&lt; 1s</p><p className="text-white/40 text-xs mt-0.5">Response time</p></div>
                  </div>
                </div>
              </div>
            )}
...
function ROICalculator() {
  ...
  // Current workforce cost (in USD)
  const currentCostUSD = (agents * avgSalary * utilization) / 100;
  const agentixPlatformCostUSD = agents * 500 * 12;
  const remainingHumanCostUSD = currentCostUSD * 0.20;
  const totalAgentixCostUSD = agentixPlatformCostUSD + remainingHumanCostUSD;
  const rawSavingsUSD = currentCostUSD - totalAgentixCostUSD;
  const savingsUSD = Math.max(rawSavingsUSD, currentCostUSD * 0.40);
  const savingsPercent = ((savingsUSD / currentCostUSD) * 100).toFixed(1);
  const paybackMonths = Math.max(1, (totalAgentixCostUSD / (savingsUSD / 12))).toFixed(1);

  // Convert to display currency
  const fx = currency === 'ILS' ? USD_TO_ILS : 1;
  ...
  const salaryDisplay = currency === 'ILS'
    ? `₪${((avgSalary * USD_TO_ILS) / 1000).toFixed(0)}K`
    : `$${(avgSalary / 1000).toFixed(0)}K`;

  const salaryMin = currency === 'ILS' ? '₪92K' : '$25K';
  const salaryMax = currency === 'ILS' ? '₪370K' : '$100K';
          ...
          <div className="bg-gradient-to-br from-[#2563EB]/5 to-[#2563EB]/10 border border-[#2563EB]/20 rounded-2xl p-8 reveal">
            ...
            <div className="flex justify-end mb-8">
              <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-xl p-1 shadow-sm">
                <span className="text-xs text-gray-500 px-2" style={{ fontFamily: "Inter, sans-serif" }}>Currency:</span>
                {(['USD', 'ILS'] as const).map((c) => (
                  <button
                    key={c}
                    onClick={() => setCurrency(c)}
                    className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all ${
                      currency === c ? 'bg-[#2563EB] text-white shadow-sm' : 'text-gray-500 hover:text-gray-800'
                    }`}
                    style={{ fontFamily: "Space Grotesk, sans-serif" }}
                  >
                    {c === 'USD' ? '$ USD' : '₪ ILS'}
            ...
            <div className="grid md:grid-cols-3 gap-8 mb-12">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full h-2 bg-gray-300 rounded-lg appearance-none cursor-pointer accent-[#2563EB]"
                />
                <p className="text-xs text-gray-500 mt-2" style={{ fontFamily: "Inter, sans-serif" }}>{salaryMin} to {salaryMax}</p>
              </div>
                  ...
                  <button
                    onClick={async () => {
                      if (email.match(/^[^\s@]+@[^\s@]+\.[^\s@]+$/)) {
                        try {
                          await fetch("https://agentix-backend-bpgg.onrender.com/api/roi", {
                            method: "POST",
                            headers: { "Content-Type": "application/json" },
                            body: JSON.stringify({ email, agents, avgSalary, utilization, currency }),
                          });
                        } catch (e) {
